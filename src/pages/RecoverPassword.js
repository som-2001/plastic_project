...
export const RecoverPassword=()=>{
   ...
   const submit=()=>{
      axios.post('http://localhost:3001/confirmPassword',{password:password,newPassword:newPassword}).then(res=>{
    ...
    <div style={{width:"30rem",height:"23rem"}} className="mb-5"> <center>
      <Slider1 />
        ...
        <div class="row col-12 col-md-6 col-sm-7 col-lg-4 g-3 mt-5 justify-content-center">
            ...
            <input type={text ? 'text':'password'} class="form-control" id="inputPassword" placeholder="Your new password" style={{maxWidth:"330px"}} onChange={(e)=>setPassword(e.target.value)}
            />
            ...
            <div class="col-10 col-lg-11 col-md-11 col-sm-11">
            <label for="inputPassword4" class="form-label"  style={{fontWeight:"100"}} >Confirm Password</label>
            <div id="eye" style={{display: "flex"}} >
            <input type={text ? 'text':'password'} class="form-control" id="inputPassword" placeholder="Confirm password" style={{maxWidth:"330px"}} onChange={(e)=>setNewPassword(e.target.value)}
            />
            <div style={{border:'0.2px solid black',borderRadius:"0px 9px 9px 0px"}}>
            <button onClick={()=>{setText(!text)}} style={{border:"none",background:"none",fontSize:"1.4rem"}}>{text?<FaEye/>:<FaEyeSlash/>}</button>
            </div>
