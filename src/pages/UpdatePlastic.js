...
export const UpdatePlastic=()=>{
    ...
    useEffect(() => {
      Axios.get("http://localhost:3001/api/read_plastic_v2")
        .then((response) => {
    ...
    const handleChangeType=(e)=>{
      ...
      axios.post('http://localhost:3001/api/updateOthersType',{id:e}).then(res=>{
        alert('success');
      })
              ...
              <tbody>
                {results?(results.map((result, index) => (
                  <tr key={index}>
                    ...
                    <td><center><button onClick={(e)=>handleChangeType(result.picture_id)} className="btn btn-success">Change to Others</button></center></td>
                  </tr>
                ))):(
                  <div>
